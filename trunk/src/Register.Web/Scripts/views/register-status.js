//var app = angular.module("searchStatusModule", ["ui.bootstrap", "cspModule"]);
app.controller("searchStatusController", ["$scope", "CspUtils", "LookupService", "$http", function ($scope, $CspUtils, $LookupService, $http) {
    ...
    $scope.submit = function () {
        ...
        var url = TEST_TYPE_ROOT + "/Inquiry/RegisterStatus";
        $http.post(url, $scope.model, config)
                .then(function (result) {
                    if (result.data.rtn.Success) {
                        $scope.registerStatus = result.data.Data;
                        console.log($scope.registerStatus);
                        if ($scope.registerStatus) {
                            $scope.show = true;
                            $scope.isInfo = false;
                            if ($scope.registerStatus.PAYIN_BUTTON_FLAG == 'Y') {
                                $scope.getQrCode();
                            }
                        }
                        else {
